...
BEGIN
	...
	UNION all
	select count(MysteryS)				from ShowOverUsers where MysteryS = 1
	UNION all
	...
	UNION all
	select count(ModernS)				from ShowOverUsers where ModernS = 1
	UNION all
	select count(HistoricS)				from ShowOverUsers where HistoricS = 1
	UNION all
	select count(PreHistoricS)			from ShowOverUsers where PreHistoricS = 1
	UNION all
	select count(ComicsS)				from ShowOverUsers where ComicsS = 1
	UNION all
	select count(PeriodS)				from ShowOverUsers where PeriodS = 1
	UNION all
