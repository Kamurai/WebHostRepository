...
BEGIN
	...
	BEGIN
		...
		SET @GlobalExclusionCount = (select count(Shows.Indext) from Shows, ShowOverUsers
			where (
			(Genre = 'ComedyS'			and ComedyS = 1)			or 
			(Genre = 'DramaS'			and DramaS = 1)				or 
			(Genre = 'ActionS'			and ActionS = 1)			or 
			(Genre = 'HorrorS'			and HorrorS = 1)			or 
			(Genre = 'ThrillerS'		and ThrillerS = 1)			or 
			(Genre = 'ScienceFictionS'	and ScienceFictionS = 1)	or 
			(Genre = 'FantasyS'			and FantasyS = 1)			or 
			(Genre = 'WesternS'			and WesternS = 1)			or 
			(Genre = 'MartialArtsS'		and MartialArtsS = 1)		or 
			(Genre = 'DocumentaryS'		and DocumentaryS = 1)		or 
			(Genre = 'MockumentaryS'	and MockumentaryS = 1)
			) 
		...
		BEGIN
			...
			BEGIN    
				...
				select Shows.Indext, Name, Release, Genre, Picture from ShowOverLists, Shows where ShowOverLists.Indext = @TargetIndex and ShowIndex = Shows.Indext
				UNION
				--//request random from global list
					--//exclude from personal list
				select * from ( select Top 1 Shows.Indext, Name, Release, Genre, Picture from Shows, ShowOverUsers
				where (
				(Genre = 'ComedyS'			and ComedyS = 1)			or 
				(Genre = 'DramaS'			and DramaS = 1)				or 
				(Genre = 'ActionS'			and ActionS = 1)			or 
			...
			BEGIN
				...
				--//request @TargetIndex from personal list
				select Shows.Indext, Name, Release, Genre, Picture from  Shows, ShowOverLists where ShowOverLists.Indext = @TargetIndex and ShowIndex = Shows.Indext
				UNION
				--//request adjacent non-locked Target from personal list
				select * from (
					select top 1 Shows.Indext, Name, Release, Genre, Picture from Shows, ShowOverLists where UserIndex = @intUserIndex and Shows.Indext = ShowIndex and ( (OrderRank = @SavedOrder-1 and DownLock = 0) or (OrderRank = @SavedOrder+1 and UpLock = 0) ) order by newid()
				) T2;
			END
		END                    
		...
		BEGIN
			...
			BEGIN
				...
				select * from (
				select top 1 Shows.Indext, Name, Release, Genre, Picture from Shows, ShowOverLists where UserIndex = @intUserIndex and ShowIndex = Shows.Indext and ( OrderRank = 0 or OrderRank = @UserCount-1 )
				order by newid() ) T3
				UNION
				--//request random from global list
					--//exclude from personal list
			...
			BEGIN
				...
				select top 2 Shows.Indext, Name, Release, Genre, Picture from Shows, ShowOverLists where UserIndex = @intUserIndex
	...
	BEGIN
		...
		select top 2 Shows.Indext, Name, Picture, Release, Genre from Shows, ShowOverUsers where ShowOverUsers.UserIndex = @intUserIndex and 
		(
		(Genre = 'ComedyS'			and ComedyS = 1)			or 
		(Genre = 'DramaS'			and DramaS = 1)				or 
		(Genre = 'ActionS'			and ActionS = 1)			or 
